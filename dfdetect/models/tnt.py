# From https://github.com/lucidrains/transformer-in-transformer/blob/4ca273442024698075955475c6cc4f61ae894dd3/transformer_in_transformer/tnt.py
...
class TNT(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = repeat(self.patch_tokens[: (n + 1)], "n d -> b n d", b=b) + rearrange(
            self.patch_pos_emb[: (n + 1)], "n d -> () n d"
        )

        # patches += torch.unsqueeze(
        #     self.patch_pos_emb[: (n + 1)], dim=0
        # )  #
        ...
        cls_token = patches[:, 0]
        return self.mlp_head(cls_token) if self.mlp_head is not None else cls_token
